...
import { Alert, KeyboardAvoidingView, Platform, StyleSheet, ScrollView, View, Pressable, } from "react-native";
...
import { hasBiometricCredentials, saveBiometricCredentials, getBiometricCredentials, clearBiometricCredentials, } from "../../lib/biometric-credentials";
export default function SignIn() {
    ...
    const performSignIn = async (loginEmail: string, loginPassword: string, opts?: {
        ...
        try {
            await authClient.signIn.email({
                ...
                onSuccess: async () => {
                    dismissAuthBrowser();
                    const name = useAuthStore.getState().session?.user?.name ??
    ...
    const promptEnableBiometric = (loginEmail: string, loginPassword: string) => {
        Alert.alert("Enable Biometric Login", "Use Face ID / fingerprint to log in next time?", [
            {
                text: "Not Now",
                style: "cancel",
    ...
    return (<View style={styles.container}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={["left", "right", "bottom"]}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} bounces={false} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            styles.inputWrapper,
            {
                ...
                borderWidth: isDark ? 0 : 1,
                  ...
                  <TextInput mode="flat" value={email} onChangeText={setEmail} placeholder="canandoe@gmail.com" placeholderTextColor={colors.inputPlaceholder} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} style={styles.textInput} textColor={colors.inputText} disabled={loading} underlineColor="transparent"/>
                </View>
              </View>
            ...
            styles.inputWrapper,
            {
                ...
                borderWidth: isDark ? 0 : 1,
                  ...
                  <TextInput mode="flat" value={password} onChangeText={setPassword} placeholder="••••••••••••" placeholderTextColor={colors.inputPlaceholder} autoCapitalize="none" autoCorrect={false} secureTextEntry={!showPass} style={styles.textInput} textColor={colors.inputText} underlineColor="transparent" disabled={loading}/>
                  <Pressable onPress={() => setShowPass((v) => !v)} style={styles.eyeButton}>
                    <MaterialCommunityIcons name={showPass ? "eye-off-outline" : "eye-outline"} size={20} color={colors.iconColor}/>
                  </Pressable>
                </View>
              </View>
            </View>
        ...
        ]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ borderRadius: 12, flex: 1 }}>
                <HapticPressable onPress={handleSignIn} disabled={loading}>
                  <Button mode="contained" contentStyle={{ height: 52 }} style={{ borderRadius: 16, backgroundColor: "transparent" }} labelStyle={styles.buttonText}>
                    {loading ? "Signing in..." : "Login"}
                  </Button>
                </HapticPressable>
              </LinearGradient>
                <HapticPressable style={{
            marginRight: 10,
            borderWidth: 1,
            padding: 8,
        ...
        }} disabled={loading || biometricLoading} onPress={handleBiometricLogin}>
                <MaterialCommunityIcons name="face-recognition" size={40} color={hasSavedBiometric ? theme.colors.primary : colors.iconColor}/>
              </HapticPressable>
            </View>
