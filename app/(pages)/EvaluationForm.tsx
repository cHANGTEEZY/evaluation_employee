import { StyleSheet, Text as RNText, View, KeyboardAvoidingView, Platform, Alert, Modal, } from "react-native";
import React, { useState, useEffect, useMemo } from "react";
import { SafeAreaView, useSafeAreaInsets, } from "react-native-safe-area-context";
import Animated, { useSharedValue, useAnimatedStyle, withRepeat, withTiming, Easing, } from "react-native-reanimated";
import { Button, ProgressBar, useTheme, ActivityIndicator, IconButton, Text, } from "react-native-paper";
import { FormProvider, useForm, FieldPath, SubmitHandler, Resolver, } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { valuationSchema, ValuationFormValues, defaultValuationValues, cleanValuationValues, } from "../../constants/form-schema";
import { insertValuation, updateValuationStatus, updateValuation, getValuationById, rowToFormValues, getPaymentsByValuationId, getRefNosStartingWith, } from "../../lib/schema";
import { AuthenticationError } from "../../lib/auth-guard";
import { useRouter, useLocalSearchParams } from "expo-router";
import { generatePaymentReceipt, sharePaymentReceipt, receiptFileExists, PaymentReceiptData, } from "../../lib/pdf-generator";
...
function SubmitOverlay({ visible }: {
    visible: boolean;
}) {
    ...
    return (<Modal visible={visible} transparent animationType="fade" onRequestClose={() => { }}>
      <View style={styles.overlay}>
        <View style={[
          ...
          <Text variant="titleMedium" style={[styles.submitText, { color: theme.colors.onSurface }]}>
            Submitting…
          </Text>
        </View>
      </View>
...
const EvaluationForm = () => {
    ...
    useEffect(() => {
        const generateRefNo = async () => {
            ...
            if (currentRef && currentRef.startsWith(base))
                return;
            try {
    ...
    const getStepForField = (field: FieldPath<ValuationFormValues>): number => {
        for (let step = 0; step < TOTAL_STEPS; step++) {
            if (getFieldsForStep(step).includes(field))
                return step;
        }
        return 0;
    };
    ...
    const canSubmit = useMemo(() => {
        const hasRequiredFields = (formValues.client_name ?? "").toString().trim().length > 0 &&
    ...
    const handleSubmitWithValidation = () => {
        form.handleSubmit(onSubmit, (errors) => {
            const firstErrorKey = Object.keys(errors)[0] as FieldPath<ValuationFormValues> | undefined;
            const firstErrorObj = firstErrorKey
                ? (errors as Record<string, {
                    message?: string;
                }>)[firstErrorKey]
                : undefined;
            const firstError = firstErrorObj?.message ?? "Please complete required fields.";
    ...
    const handleSaveDraft = async () => {
        try {
            ...
            if (error instanceof AuthenticationError) {
                Alert.alert("Authentication Required", "You must be logged in to save drafts. Please sign in and try again.", [{ text: "OK", onPress: () => router.replace("/(auth)/login") }]);
            }
            else {
                toast({
    ...
    const handleViewReceipt = async () => {
        ...
        try {
            ...
            if (!exists) {
                Alert.alert("Receipt not found", "The payment receipt PDF is no longer on this device. It may have been deleted or this valuation was opened on another device.");
                return;
            }
        ...
        try {
            ...
            if (data.site_charge && data.site_charge > 0) {
                try {
                    ...
                    const existingPaymentId = existingPayments.length > 0 ? existingPayments[0].id : undefined;
                    ...
                    pdfUri = await generatePaymentReceipt(valuationId, receiptData, existingPaymentId);
                    console.log(existingPaymentId
                        ? "PDF receipt updated:"
                        : "PDF receipt generated:", pdfUri);
                }
                catch (pdfError) {
            ...
            if (pdfUri) {
                toast({
                    title: "Saved",
                    message: "Receipt saved. Open it from Evaluations → valuation → View receipt.",
                    preset: "done",
                });
            }
            ...
            if (error instanceof AuthenticationError) {
                Alert.alert("Authentication Required", "You must be logged in to save evaluations. Please sign in and try again.", [
                    {
                        text: "OK",
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 3:
                return <Step3 />;
            case 4:
                return (<Step4 onDrawingSaved={(uri) => {
                        form.setValue("site_plan_drawing", uri);
                        setDrawingSaved(true);
                    }}/>);
            case 5:
                return (<Step5 onImagesChange={(images) => {
    ...
    if (showCompass) {
        return (<SafeAreaView edges={["left"]} style={[styles.safeArea, { backgroundColor: theme.colors.surface }]}>
      ...
      <View style={[styles.container, { paddingTop: inset.top + 10 }]}>
        <View style={styles.header}>
          ...
          <View style={styles.headerRow}>
            <IconButton icon="arrow-left" size={24} onPress={goBack} style={{ marginLeft: -8 }}/>
            <Text style={[styles.title, { color: theme.colors.primary, flex: 1 }]} numberOfLines={1}>
              {isEditMode ? "Edit Valuation" : "Property Valuation Form"}
            </Text>
            <Button mode="contained" onPress={handleSubmitWithValidation} compact loading={isSubmitting} disabled={!canSubmit || isSubmitting} style={styles.headerSubmit} contentStyle={styles.headerSubmitContent}>
              Submit
            </Button>
            {receiptUri && (<IconButton icon="file-document-outline" size={24} onPress={handleViewReceipt} iconColor={theme.colors.primary} style={{ marginRight: -8 }}/>)}
          </View>
        ...
        <View style={[
          ...
          <Button mode="outlined" onPress={handleBack} disabled={currentStep === 0} style={styles.button} icon="arrow-left">
            Back
          </Button>

          <Button mode="outlined" onPress={handleSaveDraft} style={styles.button} icon="content-save">
            Save Draft
          </Button>

          {currentStep < TOTAL_STEPS - 1 ? (<Button mode="contained" onPress={handleNext} style={styles.button} icon="arrow-right" contentStyle={styles.buttonContent} loading={isValidating} disabled={isValidating}>
              Next
            </Button>) : (<Button mode="contained" onPress={handleSubmitWithValidation} style={styles.button} icon="check" contentStyle={styles.buttonContent} loading={isSubmitting} disabled={isSubmitting || propertyImages.length < 4}>
              Submit
            </Button>)}
        </View>
      </View>
