...
import { ModeToggle, UnitInput, UnitResultSection, } from "../../components/nepal-unit-converter";
import type { ConverterMode } from "../../components/nepal-unit-converter";
import { AREA_UNITS, AREA_UNIT_GROUPS, LENGTH_UNITS, LENGTH_UNIT_GROUPS, areaToSquareMeters, squareMetersToArea, lengthToMeters, metersToLength, metersToCompoundNepaliLength, squareMetersToCompoundHill, squareMetersToCompoundTerai, } from "../../lib/nepal-unit-converter";
...
const NepalUnitConverter = () => {
    ...
    const handleUnitCardPress = useCallback((unitKey: string) => {
        if (mode === "area") {
            setFromUnitArea(unitKey as AreaUnitKey);
            setInputValue(formatDisplayValue(areaResultsExact[unitKey as AreaUnitKey] ?? 0));
        }
        else {
            setFromUnitLength(unitKey as LengthUnitKey);
            setInputValue(formatDisplayValue(lengthResultsExact[unitKey as LengthUnitKey] ?? 0));
        }
    ...
    return (<View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <LinearGradient colors={[theme.colors.primary, theme.colors.primaryContainer]} start={{ x: 1, y: 0 }} end={{ x: 0, y: 0.5 }} style={[
            styles.header,
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, { paddingBottom: 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.displayModeToggleRow}>
          <Pressable onPress={() => setDisplayMode("exact")} style={[
          ...
          <Pressable onPress={() => setDisplayMode("relative")} style={[
            styles.displayModeBtn,
        ...
        <UnitInput value={inputValue} onValueChange={setInputValue} onNumericChange={handleNumericChange} units={units} selectedUnitId={fromUnit} onUnitSelect={handleUnitSelect}/>

        {mode === "area" && (<>
            <UnitResultSection title="Hill Region (Ropani System)" icon="image-filter-hdr" unitKeys={AREA_UNIT_GROUPS.hill} results={areaResultsRelative} fromUnit={fromUnitArea} formatValue={displayMode === "relative" ? formatCompoundValue : formatDisplayValue} getUnitLabel={getAreaLabel} onUnitPress={handleUnitCardPress}/>
            <UnitResultSection title="Terai Region (Bigha System)" icon="sprout" unitKeys={AREA_UNIT_GROUPS.terai} results={areaResultsRelative} fromUnit={fromUnitArea} formatValue={displayMode === "relative" ? formatCompoundValue : formatDisplayValue} getUnitLabel={getAreaLabel} onUnitPress={handleUnitCardPress}/>
            <UnitResultSection title="Standard Units" icon="ruler-square" unitKeys={AREA_UNIT_GROUPS.standard} results={areaResultsExact} fromUnit={fromUnitArea} formatValue={formatDisplayValue} getUnitLabel={getAreaLabel} onUnitPress={handleUnitCardPress}/>
          </>)}

        {mode === "length" && (<>
            <UnitResultSection title="Nepali Traditional Units" icon="ruler" unitKeys={LENGTH_UNIT_GROUPS.nepali} results={lengthResultsRelative} fromUnit={fromUnitLength} formatValue={displayMode === "relative" ? formatCompoundValue : formatDisplayValue} getUnitLabel={getLengthLabel} onUnitPress={handleUnitCardPress}/>
            <UnitResultSection title="Standard Units" icon="ruler-square" unitKeys={LENGTH_UNIT_GROUPS.standard} results={lengthResultsExact} fromUnit={fromUnitLength} formatValue={formatDisplayValue} getUnitLabel={getLengthLabel} onUnitPress={handleUnitCardPress}/>
          </>)}
      </ScrollView>
    </View>);
};
