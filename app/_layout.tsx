...
export default function RootLayout() {
    ...
    return (<>
      ...
      {showAnimatedSplash && (<AnimatedSplashScreen onFinish={handleAnimatedSplashFinish} minDisplayTime={ANIMATED_SPLASH_MIN_TIME}/>)}
    </>);
}
...
function RootLayoutNav() {
    ...
    const toastConfig = useMemo(() => {
        const render = (variant: AppToastVariant) => (props: any) => (<AppToast {...props} variant={variant} themeMode={effectiveTheme}/>);
        return {
    ...
    return (<GestureHandlerRootView style={{ flex: 1 }}>
      <Suspense fallback={<ActivityIndicator size={"large"}/>}>
        <ThemeProvider value={effectiveTheme === "dark" ? DarkTheme : DefaultTheme}>
          <SQLiteProvider databaseName="evaluationapp" options={{
            ...
            <ToastManager config={toastConfig} position="top" width="96%" minHeight={64} duration={3200} animationStyle="fade" theme={effectiveTheme} iconFamily="MaterialCommunityIcons" iconSize={22} showCloseIcon={false} showProgressBar={false} useModal={false} topOffset={36} style={styles.toastManager}/>
          </SQLiteProvider>
        </ThemeProvider>
      </Suspense>
...
function AppToast({ text1, text2, hide, variant, themeMode }: AppToastProps) {
    ...
    return (<View style={[
      ...
      <View style={[styles.toastIcon, { backgroundColor: palette.tint }]}>
        <MaterialCommunityIcons name={palette.icon as any} size={20} color={palette.border}/>
      </View>
      ...
      <Pressable onPress={hide} style={styles.toastClose} hitSlop={8}>
        <MaterialCommunityIcons name="close" size={18} color={isDark ? "#94A3B8" : "#475569"}/>
      </Pressable>
    </View>);
}
