...
import { getAllValuations, ValuationRow, seedDummyValuation, } from "../../lib/schema";
...
export default function EvaluationsScreen() {
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchText) {
            ...
            filtered = filtered.filter((item) => {
                ...
                const address = item.present_property_address?.toLowerCase() ||
    ...
    return (<View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      <LinearGradient colors={[theme.colors.secondaryContainer, theme.colors.primary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0.5 }} style={[styles.gradientHeader, { paddingTop: insets.top + 12 }]}>
        <View style={styles.headerContent}>
          ...
          {__DEV__ && (<IconButton icon="test-tube" iconColor="white" size={22} onPress={handleSeedDummy} style={{ margin: 0 }}/>)}
          <IconButton icon="menu" iconColor="white" size={24} onPress={() => setDrawerVisible(true)} style={{ margin: 0 }}/>
        </View>

        <Searchbar placeholder="Search valuations..." value={searchText} onChangeText={(text) => setSearchText(text)} iconColor={theme.colors.onSurfaceVariant} inputStyle={{ color: theme.colors.onSurface, fontSize: 15 }} traileringIcon={searchText ? "close" : ""} onTraileringIconPress={() => setSearchText("")} style={[
            styles.searchBar,
            {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        ...
        {loading ? (<View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          ...
          </View>) : filteredData.length === 0 ? (<View style={{
                flex: 1,
            ...
            <View style={[
              ...
              <MaterialCommunityIcons name="clipboard-text-search-outline" size={48} color={theme.colors.onSurfaceVariant}/>
            </View>
            <Text variant="titleMedium" style={{
                color: theme.colors.onSurface,
                marginTop: 16,
          ...
          </View>) : (<FlashList data={filteredData} contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false} renderItem={({ item }) => {
                const displayStatus = getStatusDisplay(item.status, item.sync_status);
                return (<Card mode="elevated" elevation={0} style={styles.cardItem} onPress={() => router.push({
                  ...
                  <View style={styles.cardContent}>
                    <View style={[
                        styles.iconBox,
                        {
                            backgroundColor: displayStatus === "Draft"
                      ...
                      <MaterialCommunityIcons name={displayStatus === "Draft"
                        ? "file-document-edit-outline"
                        : displayStatus === "Pending"
                            ? "clock-outline"
                            : displayStatus === "submitted"
                                ? "check-circle-outline"
                                : "cloud-check-outline"} size={24} color={displayStatus === "Draft"
                    ...
                    <View style={{ flex: 1, marginRight: 12 }}>
                      ...
                      <View style={{
                        ...
                        <MaterialCommunityIcons name="map-marker-outline" size={14} color={theme.colors.outline}/>
