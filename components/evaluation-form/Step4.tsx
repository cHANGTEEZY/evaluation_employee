import { StyleSheet, View, Alert, Image, ScrollView, Pressable, Text as RNText, } from "react-native";
import React, { useEffect, useRef, useState } from "react";
import { Button, Divider, Surface, Text, useTheme } from "react-native-paper";
import { File, Paths } from "expo-file-system";
import { useFormContext } from "react-hook-form";
import ViewShot from "react-native-view-shot";
import PropertyPlotter, { type PlotterData, type PlotterUIState, type PlotTriangle, type PropertyPlotterRef, type MeasureUnit, type TapMode, } from "../PropertyPlotter";
import { UnitResultSection } from "../nepal-unit-converter";
import { AREA_UNITS, AREA_UNIT_GROUPS, squareMetersToArea, squareMetersToCompoundHill, squareMetersToCompoundTerai, } from "../../lib/nepal-unit-converter";
function distInFt(d: {
    feet: number;
    inches: number;
    totalFt?: number;
    ...
    if (!d)
        return null;
    if (d.totalFt != null && d.totalFt > 0)
        return d.totalFt;
    const total = d.feet + d.inches / 12;
    return total > 0 ? total : null;
}
function heronSqFt(a: number | null, b: number | null, c: number | null): number | null {
...
const Step4 = ({ onDrawingSaved }: Step4Props) => {
    ...
    const [plotterData, setPlotterData] = useState<PlotterData | null>(initialPlotterData);
    const [isSaving, setIsSaving] = useState(false);
    const [savedUri, setSavedUri] = useState<string | null>(existingDrawing || null);
    ...
    const [areaDisplayMode, setAreaDisplayMode] = useState<"exact" | "relative">("exact");
    ...
    const handleSaveDrawing = async () => {
        if (!hasPoints) {
            Alert.alert("No Drawing", "Please place at least one point before saving.");
            return;
        }
        try {
            setIsSaving(true);
    ...
    const handleClearDrawing = () => {
        Alert.alert("Clear Site Plan?", "This will remove all points and triangles. Are you sure?", [
    ...
    return (<ScrollView style={styles.scrollContainer} contentContainerStyle={styles.container}>
      ...
      <View style={styles.plotterToolbar}>
        <Button mode={plotterUIState.measureUnit === "imperial"
            ? "contained-tonal"
            : "outlined"} icon="ruler" onPress={() => plotterRef.current?.setMeasureUnit(plotterUIState.measureUnit === "imperial" ? "metric" : "imperial")} compact style={styles.toolbarBtn}>
          {plotterUIState.measureUnit === "imperial" ? "ft/in" : "m/cm"}
        </Button>

        <Button mode={plotterUIState.tapMode === "delete" ? "contained" : "outlined"} icon={plotterUIState.tapMode === "delete" ? "cursor-pointer" : "delete"} onPress={() => plotterRef.current?.setTapMode(plotterUIState.tapMode === "select" ? "delete" : "select")} compact buttonColor={plotterUIState.tapMode === "delete" ? theme.colors.error : undefined} textColor={plotterUIState.tapMode === "delete" ? "white" : theme.colors.error} style={[styles.toolbarBtn, { borderColor: theme.colors.error }]}>
          {plotterUIState.tapMode === "delete" ? "Delete On" : "Delete"}
        </Button>

        <Button mode="outlined" icon="undo" onPress={() => plotterRef.current?.undo()} disabled={plotterUIState.pointCount === 0} compact textColor={theme.colors.tertiary} style={[styles.toolbarBtn, { borderColor: theme.colors.outline }]}>
          Undo
        </Button>

        {plotterUIState.selectedCount > 0 && (<Button mode="outlined" icon="close" onPress={() => plotterRef.current?.deselectAll()} compact textColor={theme.colors.error} style={[styles.toolbarBtn, { borderColor: theme.colors.error }]}>
            {`Deselect (${plotterUIState.selectedCount})`}
          </Button>)}
      </View>

      
      <ViewShot ref={captureViewRef} options={{ format: "png", quality: 1 }} style={styles.viewShotWrapper}>
        <View style={styles.canvasContainer}>
          {showSavedPreview ? (<>
              <Image source={{ uri: savedUri }} style={styles.savedImage} resizeMode="contain"/>
              <View style={[
                styles.savedOverlay,
            ...
            </>) : (<PropertyPlotter ref={plotterRef} onDataChange={setPlotterData} onUIStateChange={setPlotterUIState} initialData={initialPlotterData}/>)}
        </View>

        
        {!showSavedPreview && hasTriangles && plotterData && (<TextDataOverlay data={plotterData} displayMode={areaDisplayMode}/>)}
      </ViewShot>

      
      {!showSavedPreview && hasTriangles && plotterData && (<MeasurementPanel data={plotterData} displayMode={areaDisplayMode} onDisplayModeChange={setAreaDisplayMode}/>)}

      {!!savedUri && hasPoints && (<View style={[
      ...
      <View style={styles.buttonContainer}>
        <Button mode="outlined" onPress={handleClearDrawing} disabled={!hasPoints && !savedUri} style={styles.button} icon="eraser">
          Clear
        </Button>
        <Button mode="contained" onPress={handleSaveDrawing} loading={isSaving} disabled={isSaving || !hasPoints} style={styles.button} icon="content-save">
...
function TextDataOverlay({ data, displayMode }: TextDataOverlayProps) {
    ...
    const areaResults = displayMode === "relative"
        ? {
            ...areaResultsExact,
    ...
    return (<View style={styles.textOverlay}>
      ...
      {results.map((r) => {
            ...
            return (<View key={r.tri.id} style={styles.textTriBlock}>
            ...
            {r.areaSqFt != null && r.areaSqM != null ? (<RNText style={styles.textTriArea}>
                → Area: {rawValue(r.areaSqFt)} sq ft ({rawValue(r.areaSqM)} m²)
              </RNText>) : (<RNText style={styles.textTriArea}>→ Area: Incomplete</RNText>)}
          </View>);
        })}
...
function MeasurementPanel({ data, displayMode, onDisplayModeChange, }: MeasurementPanelProps) {
    const { triangles } = data;
    const theme = useTheme();
    interface TriangleResult {
        tri: PlotTriangle;
    ...
    return (<Surface style={[styles.detailsPanel, { backgroundColor: cardBg }]} elevation={1}>
      <Text variant="titleSmall" style={[styles.detailsTitle, { color: accentColor }]}>
        Area Details
      </Text>
      ...
      {results.map((r) => {
            const [ai, bi, ci] = r.tri.pointIndices;
            const allSidesPresent = r.sides[0] != null && r.sides[1] != null && r.sides[2] != null;
            const complete = allSidesPresent && r.areaSqFt != null;
            return (<View key={r.tri.id} style={styles.triRow}>
            <View style={styles.triHeader}>
              <Text variant="labelLarge" style={{ color: accentColor, fontWeight: "700" }}>
                Triangle {r.index + 1}
              </Text>
            ...
            <View style={styles.sidesRow}>
              {([0, 1, 2] as const).map((s) => (<View key={s} style={styles.sideCell}>
                  ...
                  <Text variant="bodySmall" style={{
                        color: r.sides[s] != null ? valueColor : theme.colors.error,
                        fontWeight: "600",
                    }}>
                    {sideLabel(r.sides[s])}
                  </Text>
            ...
            {complete && r.areaSqFt != null && r.areaSqM != null ? (<View style={styles.areaBlock}>
                <View style={styles.areaRow}>
                  <Text variant="labelSmall" style={{ color: labelColor }}>
                    Area
                  </Text>
                  <Text variant="bodyMedium" style={{ color: valueColor, fontWeight: "700" }}>
              ...
              </View>) : allSidesPresent ? (<Text variant="bodySmall" style={{ color: theme.colors.error, marginTop: 4 }}>
                Invalid triangle — these side lengths don't form a valid
                triangle. Please re-check your measurements.
              </Text>) : (<Text variant="bodySmall" style={{ color: theme.colors.error, marginTop: 4 }}>
      ...
      {measuredCount > 0 && (<View style={[
          ...
          <View style={styles.displayModeToggleRow}>
            <Pressable onPress={() => onDisplayModeChange("exact")} style={[
            ...
            <Pressable onPress={() => onDisplayModeChange("relative")} style={[
                styles.displayModeBtn,
          ...
          <UnitResultSection title="Hill Region (Ropani System)" icon="image-filter-hdr" unitKeys={AREA_UNIT_GROUPS.hill} results={areaResults} fromUnit="ropani" formatValue={formatValue} getUnitLabel={getUnitLabel}/>
          <UnitResultSection title="Terai Region (Bigha System)" icon="sprout" unitKeys={AREA_UNIT_GROUPS.terai} results={areaResults} fromUnit="bigha" formatValue={formatValue} getUnitLabel={getUnitLabel}/>
          <UnitResultSection title="Standard Units" icon="ruler-square" unitKeys={AREA_UNIT_GROUPS.standard} results={areaResults} fromUnit="square_feet" formatValue={formatValue} getUnitLabel={getUnitLabel}/>
        </View>)}

      {measuredCount === 0 && (<Text variant="bodySmall" style={{ color: labelColor, textAlign: "center", marginTop: 4 }}>
