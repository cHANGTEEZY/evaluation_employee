...
import { ScrollView, StyleSheet, View, Pressable, Modal, } from "react-native";
...
const DocumentRow = ({ docKey, label, icon }: DocumentRowProps) => {
    ...
    const ToggleChip = ({ isSelected, label, onPress, isOriginal, }: {
        isSelected: boolean;
        label: string;
        onPress: () => void;
        isOriginal?: boolean;
      ...
      <MaterialCommunityIcons name={isSelected ? "check-circle" : "circle-outline"} size={18} color={isSelected
            ? isOriginal
                ? theme.colors.primary
                : theme.colors.secondary
            : theme.colors.outline}/>
    ...
    return (<View style={[
      ...
      <View style={styles.toggleContainer}>
        <Controller control={control} name={`documents.${docKey}.original`} render={({ field: { value } }) => (<ToggleChip isSelected={!!value} label="Original" isOriginal={true} onPress={() => setValue(`documents.${docKey}.original`, !value)}/>)}/>
        <Controller control={control} name={`documents.${docKey}.photocopy`} render={({ field: { value } }) => (<ToggleChip isSelected={!!value} label="Photocopy" isOriginal={false} onPress={() => setValue(`documents.${docKey}.photocopy`, !value)}/>)}/>
      </View>
    </View>);
};
function DocumentPhotosField() {
    ...
    return (<View style={styles.documentPhotosSection}>
      <Button mode="outlined" onPress={() => setModalVisible(true)} icon="camera-plus" style={styles.documentPhotosButton}>
        {photos.length > 0
      ...
      <Modal visible={modalVisible} animationType="slide" onRequestClose={() => setModalVisible(false)} statusBarTranslucent>
        <View style={[
          ...
          <PhotoCaptureScreen images={photos} onImagesChange={handleImagesChange} minImages={0} title="Document photos" helperText="Tap thumbnail to view • Tap delete in preview to remove. No limit." showCloseButton onClose={() => setModalVisible(false)}/>
        </View>
      </Modal>
    </View>);
}
const Step3 = () => {
    ...
    const numStoreys = typeof numberOfStoreys === "number" && numberOfStoreys >= 1
        ? Math.min(Math.floor(numberOfStoreys), 99)
        : 0;
    useEffect(() => {
        if (!showBuildingDetails || numStoreys <= 0)
            return;
        const current = (existingRates as (number | undefined)[]) ?? [];
        if (current.length === numStoreys)
            return;
        const next: (number | undefined)[] = Array.from({ length: numStoreys }, (_, i) => (i < current.length ? current[i] : undefined));
    ...
    return (<ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      {showBuildingDetails && (<>
          ...
          <FormSelect name="building_type" label="Building Type" options={buildingTypeOptions}/>
          <FormSelect name="building_purpose" label="Building Purpose" options={buildingPurposeOptions}/>
          <FormInput name="number_of_storeys" label="Number of storeys" keyboardType="numeric"/>
          <FormInput name="storey_height" label="Storey height (ft)" keyboardType="decimal-pad"/>
          {numStoreys > 0 && (<View style={styles.ratePerSqftSection}>
              <Text variant="titleSmall" style={styles.ratePerSqftTitle}>
                Rate per sq ft (NPR) by floor
              </Text>
              {Array.from({ length: numStoreys }, (_, i) => (<FormInput key={i} name={`building_rate_per_sqft.${i}`} label={`${getFloorLabel(i)} – Rate per sq ft (NPR)`} keyboardType="decimal-pad"/>))}
            </View>)}
        </>)}
      ...
      <View style={styles.documentsSection}>
        ...
        <Card style={styles.documentsCard} mode="outlined">
          <Card.Content style={styles.documentsCardContent}>
            {documents.map((doc, index) => (<React.Fragment key={doc.key}>
                <DocumentRow docKey={doc.key} label={doc.label} icon={doc.icon}/>
                {index < documents.length - 1 && (<Divider style={styles.divider}/>)}
              </React.Fragment>))}
          </Card.Content>
        </Card>
      </View>
      ...
      <FormInput name="remarks" label="Remarks" multiline numberOfLines={4} placeholder="Enter any additional notes..."/>
    </ScrollView>);
};
