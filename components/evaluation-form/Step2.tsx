...
const Step2 = () => {
    ...
    const rightOfWaySelectValue = (() => {
        if (rightOfWayWidthFt != null &&
            rightOfWayWidthFt !== undefined &&
            PRESET_ROW_WIDTHS.includes(rightOfWayWidthFt as (typeof PRESET_ROW_WIDTHS)[number])) {
    ...
    return (<ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <FormSelect name="property_type" label="Property Type" options={propertyTypeOptions}/>
      <FormSelect name="hold_type" label="Hold Type" options={holdTypeOptions}/>

      <Text variant="titleMedium" style={styles.sectionTitle}>
        Road & Access
      </Text>
      <FormSelect name="road_type" label="Road Type" options={roadTypeOptions}/>
      {roadType === "others" && (<FormInput name="road_type_others" label="Other road type (specify)"/>)}
      <FormInput name="road_width" label="Road Width (ft)" keyboardType="numeric"/>
      <FormSelect name="access_road_direction" label="Access Road Direction" options={accessRoadDirectionOptions}/>
      
      {accessRoadDirection === "others" && (<FormInput name="access_road_direction_others" label="Other Direction (specify)"/>)}

      <Controller control={control} name="right_of_way_m" render={({ field: { onChange } }) => (<View style={styles.selectContainer}>
            <Dropdown label="Right of Way (m)" options={RIGHT_OF_WAY_OPTIONS} value={rightOfWaySelectValue} onSelect={(v) => {
                const next = v ?? "";
                handleRightOfWaySelect(next);
                onChange(next);
            }} mode="outlined"/>
          </View>)}/>
      {rightOfWaySelectValue === "other" && (<FormInput name="right_of_way_width_ft" label="Enter right of way width (m)" placeholder="e.g. 10" keyboardType="decimal-pad"/>)}
      ...
      <FormInput name="property_area_length" label="Frontage Length" keyboardType="decimal-pad"/>
      <FormSelect name="property_frontage_direction" label="Frontage Direction" options={directionOptions}/>
      <FormInput name="property_narrowest_length" label="Narrowest Length" keyboardType="decimal-pad"/>
      <FormSelect name="property_narrowest_direction" label="Narrowest Direction" options={directionOptions}/>

      <Text variant="titleMedium" style={styles.sectionTitle}>
        Land Rates
      </Text>
      <View style={styles.unitToggleRow}>
        <Pressable onPress={() => setValue("land_rate_unit", "anna")} style={[
        ...
        <Pressable onPress={() => setValue("land_rate_unit", "kattha")} style={[
            styles.unitToggleBtn,
      ...
      <FormInput name="commercial_rate_per_anna" label={`Commercial Rate (per ${selectedRateUnit === "anna" ? "Anna" : "Kattha"})`} keyboardType="decimal-pad"/>

      
      <RiskYesNoWithSetback options={riskAreaOptions} label="Risk / Area" setbackLabel="Setback (ft)"/>

      <Text variant="titleMedium" style={styles.sectionTitle}>
        Site & Topography
      </Text>
      <FormInput name="high_land_ft" label="High Land (ft)" keyboardType="decimal-pad"/>
      <FormInput name="low_land_ft" label="Low Land (ft)" keyboardType="decimal-pad"/>
      <FormInput name="latitude" label="Latitude" keyboardType="decimal-pad"/>
      <FormInput name="longitude" label="Longitude" keyboardType="decimal-pad"/>
      <FormInput name="slope_degree" label="Slope Degree" keyboardType="decimal-pad"/>
    </ScrollView>);
};
