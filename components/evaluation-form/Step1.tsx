...
const Step1 = () => {
    ...
    const selectedBank = useMemo(() => banks.find((b) => b.name === selectedBankName) ?? null, [banks, selectedBankName]);
    const branchOptions = useMemo(() => (selectedBank?.branches ?? []).map((br) => ({
        label: br.name,
        value: br.name,
    })), [selectedBank]);
    useEffect(() => {
    ...
    return (<ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <FormSelect name="bank_name" label="Bank" options={bankOptions} placeholder="Select bank"/>
      <FormSelect name="bank_branch_name" label="Branch" options={branchOptions} placeholder="Select branch" disabled={!selectedBankName}/>
      <FormInput name="client_name" label="Client Name"/>
      <FormInput name="contact_number" label="Contact Number" keyboardType="phone-pad"/>
      <Text variant="titleMedium" style={styles.sectionTitle}>
        Property Ownership & Location
      </Text>

      <FormInput name="owner_of_property" label="Owner of Property"/>
      <FormInput name="plot_no" label="Plot No." parseAsNumber={false}/>
      <FormInput name="present_property_address" label="Present Property Address"/>
      ...
      <FormSelect name="valuation_for" label="Valuation For" options={valuationForOptions}/>
    </ScrollView>);
};
