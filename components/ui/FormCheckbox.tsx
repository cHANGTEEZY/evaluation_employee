...
const FormCheckbox = ({ name, label, disabled }: FormCheckboxProps) => {
    ...
    return (<Controller control={control} name={name} render={({ field: { onChange, value }, fieldState: { error } }) => (<View style={styles.container}>
          <TouchableOpacity style={styles.row} onPress={() => !disabled && onChange(!value)} activeOpacity={0.7}>
            <Checkbox status={value ? "checked" : "unchecked"} onPress={() => !disabled && onChange(!value)} disabled={disabled}/>
