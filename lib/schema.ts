...
import { type ValuationFormValues, normalizeBuildingRatePerSqftInput, } from "../constants/form-schema";
...
export async function createValuationTable() {
    ...
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN right_of_way_width_ft REAL;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN road_type_others TEXT;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN land_rate_unit TEXT DEFAULT 'anna';");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN document_photos TEXT;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN synced_image_hashes TEXT;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN property_evaluation_data TEXT;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN flood_prone_area INTEGER DEFAULT 0;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN flood_prone_area_setback REAL;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN building_rate_per_sqft TEXT;");
    }
    catch {
    }
    try {
        await db.execAsync("ALTER TABLE valuations ADD COLUMN site_plan_plotter_data TEXT;");
    }
    catch {
    }
}
export async function insertValuation(data: ValuationFormValues, options?: {
    employeeId?: string;
}): Promise<string> {
    const db = await getDb();
    const id = generateId();
    ...
    await db.runAsync(`INSERT INTO valuations (
...
export async function seedDummyValuation(options?: {
    ...
    const { generateClientRefNumber, generateShortId } = await import("./ref-number");
...
export async function updateValuation(id: string, data: Partial<ValuationFormValues>): Promise<void> {
    const db = await getDb();
    const now = new Date().toISOString();
    ...
    for (const mapping of fieldMappings) {
        if (mapping.key in data) {
            ...
            values.push(mapping.transform
                ? mapping.transform(value)
                : (value as string | number | null) ?? null);
        }
    }
    values.push(id);
    await db.runAsync(`UPDATE valuations SET ${updates.join(", ")} WHERE id = ?`, values);
}
export async function getValuationById(id: string): Promise<ValuationRow | null> {
    const db = await getDb();
    const result = await db.getFirstAsync<ValuationRow>("SELECT * FROM valuations WHERE id = ?", [id]);
    return result ?? null;
}
export async function getAllValuations(userId: string | null): Promise<ValuationRow[]> {
    if (userId === null)
        return [];
    const db = await getDb();
    const results = await db.getAllAsync<ValuationRow>("SELECT * FROM valuations WHERE employee_id = ? ORDER BY created_at DESC", [userId]);
    return results;
}
export async function getRecentValuations(userId: string | null): Promise<ValuationRow[]> {
    if (userId === null)
        return [];
    const db = await getDb();
    const results = await db.getAllAsync<ValuationRow>("SELECT * FROM valuations WHERE employee_id = ? ORDER BY created_at DESC LIMIT 10", [userId]);
    return results;
}
export async function getValuationsByStatus(status: "draft" | "submitted" | "synced"): Promise<ValuationRow[]> {
    const db = await getDb();
    const results = await db.getAllAsync<ValuationRow>("SELECT * FROM valuations WHERE status = ? ORDER BY created_at DESC", [status]);
    return results;
}
...
export async function getValuationsMetrics(userId: string | null): Promise<ValuationMetrics> {
    if (userId === null)
        return { total: 0, draft: 0, submitted: 0, synced: 0 };
    const db = await getDb();
    const result = await db.getFirstAsync<ValuationMetrics>("SELECT COUNT(*) AS total, COUNT(CASE WHEN status = 'draft' THEN 1 END) AS draft, COUNT(CASE WHEN status = 'submitted' THEN 1 END) AS submitted, COUNT(CASE WHEN status = 'synced' THEN 1 END) AS synced FROM valuations WHERE employee_id = ?", [userId]);
    return result ?? { total: 0, draft: 0, submitted: 0, synced: 0 };
}
export async function getRefNosStartingWith(prefix: string, excludeValuationId?: string): Promise<string[]> {
    const db = await getDb();
    if (!prefix)
        return [];
    const likePattern = prefix.replace(/\\/g, "\\\\").replace(/%/g, "\\%").replace(/_/g, "\\_") +
        "%";
    const rows = excludeValuationId
        ? await db.getAllAsync<{
            ref_no: string | null;
        }>("SELECT ref_no FROM valuations WHERE ref_no LIKE ? ESCAPE '\\' AND id != ?", [likePattern, excludeValuationId])
        : await db.getAllAsync<{
            ref_no: string | null;
        }>("SELECT ref_no FROM valuations WHERE ref_no LIKE ? ESCAPE '\\'", [likePattern]);
    return rows.map((r) => r.ref_no).filter((r): r is string => r != null);
}
export async function getPendingSyncValuations(userId: string | null): Promise<ValuationRow[]> {
    if (userId === null)
        return [];
    const db = await getDb();
    const results = await db.getAllAsync<ValuationRow>("SELECT * FROM valuations WHERE employee_id = ? AND sync_status = 'pending' ORDER BY created_at ASC", [userId]);
    return results;
}
export async function getFailedSyncValuations(userId: string | null): Promise<ValuationRow[]> {
    if (userId === null)
        return [];
    const db = await getDb();
    const results = await db.getAllAsync<ValuationRow>("SELECT * FROM valuations WHERE employee_id = ? AND sync_status = 'error' ORDER BY created_at ASC", [userId]);
    return results;
}
export async function resetFailedSyncValuations(userId: string | null): Promise<void> {
    if (userId === null)
        return;
    const db = await getDb();
    const now = new Date().toISOString();
    await db.runAsync("UPDATE valuations SET sync_status = 'pending', error_message = NULL, updated_at = ? WHERE employee_id = ? AND sync_status = 'error'", [now, userId]);
}
export async function updateValuationStatus(id: string, status: "pending" | "synced", syncStatus?: "pending" | "syncing" | "synced" | "error", errorMessage?: string): Promise<void> {
    const db = await getDb();
    const now = new Date().toISOString();
...
export async function updateSyncedImageHashes(valuationId: string, hashes: {
    ...
    await db.runAsync("UPDATE valuations SET synced_image_hashes = ?, updated_at = ? WHERE id = ?", [json, new Date().toISOString(), valuationId]);
}
export async function updateValuationServerId(valuationId: string, serverId: string | null): Promise<void> {
    const db = await getDb();
    await db.runAsync("UPDATE valuations SET server_id = ?, updated_at = ? WHERE id = ?", [serverId, new Date().toISOString(), valuationId]);
}
...
export function normalizeDocumentsForForm(raw: unknown): ValuationFormValues["documents"] {
    const doc = (raw && typeof raw === "object" ? raw : {}) as Record<string, {
        original?: boolean;
        photocopy?: boolean;
    }>;
    ...
    const nirmanSampanna = doc.nirman_sampanna || doc.nirmal_sangarna || doc.nirmarn_sangarna;
    if (nirmanSampanna) {
...
export function rowToFormValues(row: ValuationRow): Partial<ValuationFormValues> {
    return {
        ...
        access_road_direction: ((row.access_road_direction as ValuationFormValues["access_road_direction"]) ?? undefined),
        ...
        property_frontage_direction: ((row.property_frontage_direction as ValuationFormValues["property_frontage_direction"]) ?? undefined),
        property_narrowest_length: row.property_narrowest_length ?? undefined,
        property_narrowest_direction: ((row.property_narrowest_direction as ValuationFormValues["property_narrowest_direction"]) ?? undefined),
        right_of_way: row.right_of_way === 1,
        right_of_way_width_ft: row.right_of_way_width_ft ?? undefined,
        right_of_way_m: row.right_of_way_width_ft != null
        ...
        property_ownership_type: ((row.property_ownership_type as ValuationFormValues["property_ownership_type"]) ?? undefined),
        ownership_transferred_through: ((row.ownership_transferred_through as ValuationFormValues["ownership_transferred_through"]) ?? undefined),
        ...
        building_purpose: ((row.building_purpose as ValuationFormValues["building_purpose"]) ?? undefined),
        ...
        payment_online_mode: ((row.payment_online_mode as ValuationFormValues["payment_online_mode"]) ?? undefined),
...
export async function insertAuditLog(userId: string, userName: string | null, userEmail: string | null, action: string, entityType?: string, entityId?: string, details?: Record<string, any>): Promise<string> {
    const db = await getDb();
    const id = generateId();
    const now = new Date().toISOString();
    await db.runAsync(`INSERT INTO audit_logs (id, user_id, user_name, user_email, action, entity_type, entity_id, details, created_at, synced)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        id,
        userId,
        userName ?? null,
...
export async function deleteAuditLogs(ids: string[]): Promise<void> {
    ...
    await db.runAsync(`DELETE FROM audit_logs WHERE id IN (${placeholders})`, ids);
}
export async function markAuditLogsSynced(ids: string[]): Promise<void> {
    ...
    const db = await getDb();
    const placeholders = ids.map(() => "?").join(", ");
    await db.runAsync(`UPDATE audit_logs SET synced = 1 WHERE id IN (${placeholders})`, ids);
}
...
export async function insertPayment(valuationId: string, pdfUri: string, fileName: string): Promise<string> {
    const db = await getDb();
    const id = generateId();
    const now = new Date().toISOString();
    await db.runAsync(`INSERT INTO payments (id, valuation_id, pdf_uri, file_name, sync_status, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`, [id, valuationId, pdfUri, fileName, "pending", now]);
    return id;
}
export async function getPaymentsByValuationId(valuationId: string): Promise<PaymentRow[]> {
    const db = await getDb();
    const results = await db.getAllAsync<PaymentRow>("SELECT * FROM payments WHERE valuation_id = ? ORDER BY created_at DESC", [valuationId]);
    return results;
}
export async function getPendingSyncPayments(): Promise<PaymentRow[]> {
    const db = await getDb();
    const results = await db.getAllAsync<PaymentRow>("SELECT * FROM payments WHERE sync_status = 'pending' ORDER BY created_at ASC");
    return results;
}
export async function updatePaymentReceipt(paymentId: string, pdfUri: string, fileName: string): Promise<void> {
    const db = await getDb();
    await db.runAsync("UPDATE payments SET pdf_uri = ?, file_name = ? WHERE id = ?", [pdfUri, fileName, paymentId]);
}
export async function updatePaymentSyncStatus(id: string, status: "pending" | "syncing" | "synced" | "error", remoteUrl?: string): Promise<void> {
    const db = await getDb();
    const now = new Date().toISOString();
    if (remoteUrl) {
        await db.runAsync(`UPDATE payments SET sync_status = ?, remote_url = ?, uploaded_at = ? WHERE id = ?`, [status, remoteUrl, now, id]);
    }
    else {
...
export async function deletePaymentsByValuationId(valuationId: string): Promise<void> {
    const db = await getDb();
