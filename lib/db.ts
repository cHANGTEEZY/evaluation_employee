...
import { createValuationTable, createValuationImagesTable, createSyncQueue, createAuditLogsTable, createPaymentsTable, } from "./schema";
...
async function runMigrations() {
    ...
    for (const column of newColumns) {
        try {
            await database.execAsync(`ALTER TABLE valuations ADD COLUMN ${column.name} ${column.type}`);
            console.log(`Added column ${column.name} to valuations table`);
        }
        catch (error: any) {
            if (!error.message?.includes("duplicate column name")) {
                console.log(`Column ${column.name} may already exist or migration skipped`);
            }
        }
    }
}
