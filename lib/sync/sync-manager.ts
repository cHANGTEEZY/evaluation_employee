...
import { getPendingSyncValuations, getFailedSyncValuations, resetFailedSyncValuations, updateValuationStatus, getUnsyncedAuditLogs, deleteAuditLogs, getPaymentsByValuationId, updatePaymentSyncStatus, updateSyncedImageHashes, updateValuationServerId, type ValuationRow, type AuditLogEntry, } from "../schema";
...
function valuationToPayload(valuation: ValuationRow): Record<string, unknown> {
    return {
        id: valuation.id ?? "",
        employee_id: valuation.employee_id ?? "",
        ...
        road_type_others: valuation.road_type_others ?? "",
        road_width: valuation.road_width ?? "",
        access_road_direction: valuation.access_road_direction ?? "",
        access_road_direction_others: valuation.access_road_direction_others ?? "",
        ...
        right_of_way_width_ft: valuation.right_of_way_width_ft ?? "",
        ...
        landslide_prone_area_setback: valuation.landslide_prone_area_setback ?? "",
        river_side: valuation.river_side === 1,
        river_side_setback: valuation.river_side_setback ?? "",
        high_tension_area: valuation.high_tension_area === 1,
        high_tension_area_setback: valuation.high_tension_area_setback ?? "",
        canal_area: valuation.canal_area === 1,
        canal_area_setback: valuation.canal_area_setback ?? "",
        ...
        documents: valuation.documents ? JSON.parse(valuation.documents) : "",
        ...
        payment_cash: valuation.payment_cash ?? "",
        payment_online: valuation.payment_online ?? "",
        payment_online_mode: valuation.payment_online_mode ?? "",
        payment_pending_due: valuation.payment_pending_due ?? "",
        ...
        site_plan_plotter_data: valuation.site_plan_plotter_data ?? "",
        property_images: valuation.property_images
            ? JSON.parse(valuation.property_images)
            : [],
        site_plan_image: valuation.site_plan_image ?? "",
    };
}
...
export async function syncValuation(valuation: ValuationRow, authToken?: string): Promise<{
    success: boolean;
    error?: string;
    serverId?: string;
    paymentIds?: string[];
    ...
    try {
        ...
        console.log("[SYNC] Sending request to:", `${BASE_API_URL}/api/sync/valuation`, imagesUnchanged ? "(images unchanged, skipping upload)" : "");
...
export async function addToSyncQueue(entityType: "valuation" | "image", entityId: string, action: "create" | "update" | "delete"): Promise<string> {
    const db = await getDb();
    const id = `sync_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    const now = new Date().toISOString();
    await db.runAsync(`INSERT INTO sync_queue (id, entity_type, entity_id, action, attempts, last_attempt_at, status, error_message)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, entityType, entityId, action, 0, now, "pending", null]);
    return id;
}
...
export async function updateSyncQueueItem(id: string, status: "pending" | "in_progress" | "completed" | "failed", errorMessage?: string): Promise<void> {
    const db = await getDb();
    const now = new Date().toISOString();
    if (errorMessage !== undefined) {
        await db.runAsync(`UPDATE sync_queue SET status = ?, error_message = ?, last_attempt_at = ?, attempts = attempts + 1 WHERE id = ?`, [status, errorMessage, now, id]);
    }
    else {
        await db.runAsync(`UPDATE sync_queue SET status = ?, last_attempt_at = ? WHERE id = ?`, [status, now, id]);
    }
}
...
export async function resetFailedItems(): Promise<void> {
    const db = await getDb();
    await db.runAsync(`UPDATE sync_queue SET status = 'pending', error_message = NULL WHERE status = 'failed' AND attempts < ?`, [MAX_RETRY_ATTEMPTS]);
}
export async function processQueue(authToken?: string, userId?: string | null): Promise<{
    synced: number;
    failed: number;
    errors: string[];
}> {
    ...
    for (let i = 0; i < pendingValuations.length; i++) {
        ...
        await updateValuationStatus(valuation.id, valuation.status as "pending" | "synced", "syncing");
        ...
        if (result.success) {
            ...
            await updateValuationStatus(valuation.id, valuation.status as "pending" | "synced", "error", result.error);
            failed++;
...
export async function retryFailedSync(authToken?: string, userId?: string | null): Promise<{
    synced: number;
    failed: number;
    errors: string[];
}> {
...
export async function syncAuditLogs(authToken?: string): Promise<{
    synced: number;
    error?: string;
}> {
    try {
        ...
        if (data.syncedIds && data.syncedIds.length > 0) {
            ...
            console.log(`[AUDIT] Deleted ${data.syncedIds.length} synced logs from local DB`);
        }
